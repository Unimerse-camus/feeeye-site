...
- 标志含义：白色眼睛代表“看清”，瞳孔内两条等号线代表“费用可见、可比较”。
- 保留品牌蓝 `#2563eb` → 青 `#0ea5a4` 渐变；不得加入上涨/下跌箭头、K线或收益暗示。
- 唯一矢量源为 `assets/logo.svg`；`favicon-16/32/48.png`、`favicon.png` 和 `og-logo.png` 必须由该SVG机械生成，禁止分别手绘。
- 同名品牌资产受一年immutable缓存；每次替换Logo必须同步提升页面引用的查询版本（当前 `?v=2`）。
- 导航桌面尺寸30px，手机25px；favicon在16px必须仍能辨识眼睛轮廓。
