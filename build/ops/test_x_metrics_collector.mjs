...
const credentials={consumerKey:'key',consumerSecret:'secret',accessToken:'token',accessTokenSecret:'token-secret'},dataKey=Buffer.alloc(32,1),calls=[];
...
const now='2026-09-04T08:00:34.062Z';let blockedCalls=0;await assert.rejects(collectDueMetrics({storeRoot:store,credentials,now,fetchImpl:async()=>{blockedCalls++;}}),/Private data key/);assert.equal(blockedCalls,0);
const report=await collectDueMetrics({storeRoot:store,credentials,dataKey,now,fetchImpl});assert.equal(report.status,'collected');assert.equal(report.api_reads,2);assert.equal(report.snapshots[0].checkpoint,'24h');assert.equal(calls.length,2);assert.match(calls[1],/\/2\/tweets\/2095418661246800236\?tweet\.fields=/);
const encryptedFile=path.join(store,'x-metrics',key,'24h.json.enc'),envelope=JSON.parse(fs.readFileSync(encryptedFile,'utf8'));assert.equal(envelope.algorithm,'aes-256-gcm');assert.equal(JSON.stringify(envelope).includes('url_clicks'),false);const snapshot=readEncryptedJson(encryptedFile,dataKey);assert.equal(snapshot.metrics.impressions,100);assert.equal(snapshot.metrics.url_clicks,4);assert.equal(snapshot.metrics.profile_clicks,null);assert.deepEqual(snapshot.missing_fields,['profile_clicks']);assert.equal(snapshot.lag_minutes,12);
