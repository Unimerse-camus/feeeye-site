...
export async function checkLiveBilingual({baseUrl,fetchBaseUrl=baseUrl,expectedRelease,topics=LEARNING_ARTICLES.map(x=>x.slug),fetchImpl=fetch,checkedAt=new Date().toISOString()}) {
  ...
  const releaseResponse=await fetchImpl(new URL('/release.json',fetchBase));if(!releaseResponse.ok)throw new Error('release.json unavailable');const release=await releaseResponse.json();if(release.build_id!==expectedRelease.build_id||release.source_revision!==expectedRelease.source_revision)throw new Error('Live release changed after deployment receipt');
  const results=[];
