...
export async function checkLiveBilingual({baseUrl,fetchBaseUrl=baseUrl,expectedRelease,topics=LEARNING_ARTICLES.map(x=>x.slug),fetchImpl=fetch,checkedAt=new Date().toISOString(),releaseAttempts=3,releaseDelayMs=1000,sleep=ms=>new Promise(resolve=>setTimeout(resolve,ms))}) {
  const base=new URL(baseUrl),fetchBase=new URL(fetchBaseUrl);if(base.protocol!=='https:'||fetchBase.protocol!=='https:'||expectedRelease?.status!=='matched'||!validInstant(checkedAt))throw new Error('Verified deployment receipt required');
  if(!Number.isSafeInteger(releaseAttempts)||releaseAttempts<1||releaseAttempts>5||!Number.isSafeInteger(releaseDelayMs)||releaseDelayMs<0||releaseDelayMs>5000)throw new Error('Invalid release stability retry policy');
  let release=null,releaseAttemptsUsed=0;
  for(let attempt=1;attempt<=releaseAttempts;attempt++){
    const releaseUrl=new URL('/release.json',fetchBase);releaseUrl.searchParams.set('feeeye_verify',`${expectedRelease.source_revision.slice(0,12)}-${attempt}`);const response=await fetchImpl(releaseUrl,{cache:'no-store',headers:{'Cache-Control':'no-cache'}});releaseAttemptsUsed=attempt;if(response.ok){const candidate=await response.json();if(candidate.build_id===expectedRelease.build_id&&candidate.source_revision===expectedRelease.source_revision){release=candidate;break;}}
    if(attempt<releaseAttempts&&releaseDelayMs)await sleep(releaseDelayMs);
  }
  if(!release)throw new Error('Live release changed after deployment receipt');
  const results=[];
  for(const topic of topics){try{const article=LEARNING_ARTICLES.find(x=>x.slug===topic);if(!article)throw new Error('Unknown topic');const sourceUrls=article.sources.map(key=>LEARNING_SOURCES[key].url),enUrl=new URL(`/learn/${topic}`,base),zhUrl=new URL(`/zh/learn/${topic}`,base),enFetchUrl=new URL(`/learn/${topic}`,fetchBase),zhFetchUrl=new URL(`/zh/learn/${topic}`,fetchBase),responses=await Promise.all([fetchImpl(enFetchUrl),fetchImpl(zhFetchUrl)]);if(responses.some(x=>!x.ok||!(x.headers.get('content-type')||'').includes('text/html')))throw new Error('Bilingual page HTTP failure');const html=await Promise.all(responses.map(x=>x.text()));if(!html[0].includes(`hreflang="zh" href="${zhUrl}"`)||!html[1].includes(`hreflang="en" href="${enUrl}"`)||!html[0].includes(`hreflang="x-default" href="${enUrl}"`)||!html[1].includes(`hreflang="x-default" href="${enUrl}"`))throw new Error('Reciprocal hreflang mismatch');const expected={faq_count:article.en.quiz.length,section_count:article.en.sections.length,outcome_count:article.en.outcomes.length,high_risk:article.risk==='high'},en=inspect(html[0],{canonical:enUrl.toString(),language:'en',sourceUrls,expected}),zh=inspect(html[1],{canonical:zhUrl.toString(),language:'zh-CN',sourceUrls,expected});if(en.date_modified!==zh.date_modified||en.faq_count!==zh.faq_count||en.section_count!==zh.section_count||en.outcome_count!==zh.outcome_count||en.high_risk!==zh.high_risk)throw new Error('Bilingual structure or risk mismatch');results.push({topic,status:'passed',en,zh});}catch(error){results.push({topic,status:'failed',reason:error.message});}}
  const failed=results.filter(x=>x.status==='failed');return{schema_version:1,status:failed.length?'failed':'verified',base_url:base.origin,fetch_base_url:fetchBase.origin,build_id:release.build_id,source_revision:release.source_revision,checked_at:checkedAt,release_attempts_used:releaseAttemptsUsed,automatic_repair:false,distribution_allowed:failed.length===0,failed_count:failed.length,results};
}
if(process.argv[1]&&import.meta.url===pathToFileURL(path.resolve(process.argv[1])).href){const args=process.argv.slice(2),baseUrl=args[args.indexOf('--base-url')+1],fetchBaseUrl=args[args.indexOf('--fetch-base-url')+1]||baseUrl,receiptFile=args[args.indexOf('--expected-release')+1];if(!baseUrl||!receiptFile)throw new Error('Usage: --base-url URL [--fetch-base-url URL] --expected-release FILE [--all|--topic ID] [--out FILE]');const topicIndex=args.indexOf('--topic'),topics=topicIndex>=0?[args[topicIndex+1]]:LEARNING_ARTICLES.map(x=>x.slug),report=await checkLiveBilingual({baseUrl,fetchBaseUrl,expectedRelease:readJson(receiptFile),topics}),out=args.indexOf('--out');if(out>=0)writeNewJson(args[out+1],report,path.join(root,'ops/automation/working'));console.log(JSON.stringify(report,null,2));if(report.status!=='verified')process.exitCode=2;}
