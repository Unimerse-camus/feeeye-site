...
const live=await checkLiveBilingual({baseUrl:'https://feeeye.com',fetchBaseUrl:'https://feeeye-site.pages.dev',expectedRelease:{...receipt,build_id:candidate.build_id,source_revision:revision,status:'matched'},fetchImpl:fetchWithRelease,checkedAt:generatedAt});assert.equal(live.status,'verified');assert.equal(live.fetch_base_url,'https://feeeye-site.pages.dev');assert.equal(live.results.length,8);assert.equal(live.distribution_allowed,true);
