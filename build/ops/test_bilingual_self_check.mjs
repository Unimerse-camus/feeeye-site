...
const receipt=await waitForDeployment({baseUrl:'https://feeeye.com',expectedRevision:revision,expectedBuildId:candidate.build_id,attempts:3,delayMs:0,sleep:async()=>{},checkedAt:()=>generatedAt,fetchImpl:async()=>new Response(JSON.stringify(++calls<2?{build_id:'old',source_revision:'b'.repeat(40)}:{build_id:candidate.build_id,source_revision:revision,canonical_url_count:384,public_file_count:457}),{status:200,headers:{'content-type':'application/json'}})});assert.equal(receipt.attempts_used,2);
