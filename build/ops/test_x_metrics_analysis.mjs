...
assert.match(workflow,/workflow_run:/);assert.match(workflow,/workflows: \[x-metrics\]/);assert.match(workflow,/vars\.FEEEYE_X_ANALYSIS == 'enabled'/);assert.match(workflow,/group: x-receipt-ledger/);assert.doesNotMatch(workflow,/secrets\.|api\.x\.com|--execute|schedule:/);
...
const store=fs.mkdtempSync(path.join(os.tmpdir(),'feeeye-x-analysis-')),dir=path.join(store,'x-metrics','a'.repeat(64));fs.mkdirSync(dir,{recursive:true});fs.writeFileSync(path.join(dir,'24h.json'),JSON.stringify(first));assert.deepEqual(loadMetricsSnapshots(store),[first]);assert.equal(recordAnalysis(store,observed).created,true);assert.equal(recordAnalysis(store,observed).created,false);
