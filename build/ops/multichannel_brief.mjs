...
export function buildMultichannelBrief({topicId,locale,generatedAt=new Date().toISOString(),dist=path.join(root,'dist')}) {
  ...
  const contentHash=sha256(stable({topic_id:topicId,category:article.category,risk:article.risk,locale,reviewed_at:LEARNING_REVIEWED_AT,article:copy,sources,build_id:release.build_id}));
  ...
  return {schema_version:1,id:`${topicId}-${locale}-${contentHash.slice(0,12)}`,status:'draft',publishing_enabled:false,generated_at:generatedAt,topic_id:topicId,category:article.category,risk:article.risk,locale,content_hash:contentHash,source_build:{build_id:release.build_id,source_revision:release.source_revision??null,content_reviewed_at:LEARNING_REVIEWED_AT},sources,
