...
const fetchImpl=async(url,options)=>{calls.push({url,options});const body=JSON.parse(options.body);if(body.query.includes('__type(name: "Account")'))return{ok:true,status:200,json:async()=>({data:{__type:{fields:[{name:'rumPageloadEventsAdaptiveGroups',type:{kind:'LIST',name:null,ofType:{kind:'OBJECT',name:'AccountRumPageloadEventsAdaptiveGroups',ofType:null}},args:[{name:'filter',type:{kind:'INPUT_OBJECT',name:'AccountRumPageloadEventsAdaptiveGroupsFilter_InputObject',ofType:null}},{name:'limit',type:{kind:'SCALAR',name:'Int',ofType:null}}]},{name:'billing',type:{kind:'OBJECT',name:'Billing',ofType:null},args:[]}]}}})};return{ok:true,status:200,json:async()=>({data:{__type:{fields:[{name:'count',type:{kind:'SCALAR',name:'UInt64',ofType:null}},{name:'dimensions',type:{kind:'OBJECT',name:'AccountRumPageloadEventsAdaptiveGroupsDimensions',ofType:null}}]}}})};};
const result=await probeCloudflareAnalytics({token,fetchImpl,observedAt:'2026-09-04T04:00:00.000Z'});assert.equal(calls.length,2);assert.equal(calls[0].url,'https://api.cloudflare.com/client/v4/graphql');assert.equal(calls[0].options.headers.Authorization,`Bearer ${token}`);assert.deepEqual(result.datasets.map(x=>x.name),['rumPageloadEventsAdaptiveGroups']);assert.equal(result.data_rows_read,0);assert.equal(result.probe_kind,'schema_only');const summary=cloudflareProbeSummary(result);assert.equal(summary.private_details_logged,false);assert.doesNotMatch(JSON.stringify(summary),/cfat_|feeeye\.com|site_tag_bound|account_bound/);
