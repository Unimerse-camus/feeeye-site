...
`cloudflare-analytics-probe.yml`仅支持手动执行，不定时，不写仓库，不查询任何访问数据行。它只对Cloudflare GraphQL schema执行introspection，并仅保留三个预注册RUM数据集的名称、参数和类型字段。日志不输出Token、账号ID、site tag、URL、referrer或任何指标值。
